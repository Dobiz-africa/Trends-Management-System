...
export default async function handler(req,res){
  ...
  try{
    ...
    const recipients=await rest(config,`users?role=in.(${roleFilter})&is_active=eq.true&select=id,email,full_name,role`);
    ...
    for(const recipient of recipients||[]){
      ...
      const notificationId=`${id}_${recipient.role}`;
      ...
      try{
        const emailResponse=await fetch('https://api.resend.com/emails',{
          ...
          body:JSON.stringify({from,to:[recipient.email],subject:wo?`TrendsDesk action required — WO ${wo}`:'TrendsDesk notification',html:`<div style="font-family:Arial,sans-serif;max-width:560px"><h2 style="color:#075ca8">TrendsDesk</h2><p>Hello ${escapeHtml(recipient.full_name||recipient.email)},</p><p>${escapeHtml(message)}</p>${customer?`<p><strong>Customer:</strong> ${escapeHtml(customer)}</p>`:''}${wo?`<p><a href="${baseUrl}/?wo=${encodeURIComponent(wo)}" style="background:#f58220;color:#fff;padding:10px 16px;text-decoration:none;border-radius:4px">Open work order</a></p>`:''}<p style="color:#667085;font-size:12px">This automated message was sent to the ${escapeHtml(recipient.role)} role.</p></div>`})
        });
