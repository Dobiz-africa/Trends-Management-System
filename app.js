...
function renderMDDash(){
  ...
  // Jobs done = claim_docs_ready (finance finished) OR job_complete (admin confirmed)
  ...
  if(!jobs.length){wwEl.innerHTML='<div style="padding:1.5rem;text-align:center;color:var(--tx3);font-size:.8rem">No jobs yet</div>';}
    ...
    // Claim batch docs if job has a claimRef
    const claimBtns = j.claimRef && DB.batchDocs && DB.batchDocs[j.claimRef] ? [
      {dt:'annexure',lbl:'ANN'},{dt:'payment_cert',lbl:'PCert'},{dt:'invoice',lbl:'Inv'},{dt:'bpc_spreadsheet',lbl:'BPC'}
    ].map(cd=>{
      const bScans=DB.batchDocs[j.claimRef].scans||{};
      const bDocs=DB.batchDocs[j.claimRef];
      if(bScans[cd.dt]) return`<button class="btn btn-gn btn-sm" style="font-size:.62rem;padding:2px 5px" onclick="downloadScan('${j.wo}','${cd.dt}')">&#11015;${cd.lbl}&#10003;</button>`;
      if(bDocs[cd.dt]||bDocs[cd.dt==='payment_cert'?'paymentCert':cd.dt==='bpc_spreadsheet'?'bpcSpreadsheet':cd.dt]) return`<button class="btn btn-bl btn-sm" style="font-size:.62rem;padding:2px 5px" onclick="viewBatchDoc('${j.claimRef}','${cd.dt}')">&#11015;${cd.lbl}</button>`;
      return`<span style="font-size:.62rem;color:var(--tx3);padding:1px 4px;border:1px solid var(--bd);border-radius:3px;opacity:.35">${cd.lbl}</span>`;
    }).join(' ') : '';
    ...
    const allDocBtns = [docBtns, claimBtns].filter(Boolean).join(' ');
    return`<div style="padding:.65rem 1rem;border-bottom:1px solid var(--bd)">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
        <div style="flex:1;height:4px;background:var(--bd);border-radius:2px;overflow:hidden">
          <div style="width:${pct}%;height:100%;background:${pct===100?'var(--gn)':'var(--am)'};border-radius:2px;transition:width .3s"></div>
        </div>
        <span style="font-size:.65rem;color:var(--tx3);white-space:nowrap;flex-shrink:0">${pct}%</span>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:3px">${allDocBtns}</div>
    </div>`;
  }).join('');}

  // Completed jobs — jobs where finance has generated docs OR admin has confirmed complete
  const doneEl=document.getElementById('m-done-jobs');
  if(doneEl){
    if(!jobsDone.length){
      doneEl.innerHTML='<div style="padding:1.5rem;text-align:center;color:var(--tx3);font-size:.8rem">No completed jobs yet — jobs appear here once Finance generates claim documents</div>';
    } else {
      ...
      doneEl.innerHTML=`
        <div style="margin:.75rem 1rem;padding:.75rem 1rem;background:var(--gn-bg);border:1px solid var(--gn-b);border-radius:var(--rs);font-size:.78rem;color:var(--gn)">
          ...
          <div style="color:var(--tx);line-height:1.6;font-size:.76rem">
            ...
            2. <strong>Annexure to Payment Certificate</strong> — available below in Claim Batches<br>
            3. <strong>Payment Certificate</strong> — available below in Claim Batches<br>
            4. <strong>Tax Invoice</strong> — available below in Claim Batches<br>
            5. <strong>VO1 &amp; VO2 documents</strong> — signed copies from each job (download from active jobs above)<br>
            6. <strong>GIS Reports</strong> — signed copies from each job (download from active jobs above)<br>
            7. <strong>BPC Work Orders</strong> — originals received from BPC<br><br>
            Print each document → obtain hand signatures → compile the full set and submit physically to BPC offices.
          </div>
        </div>
        ${jobsDone.map(j=>{
          const t=(j.vo2&&j.vo2.items&&j.vo2.items.length)?jTotal(j,'vo2'):jTotal(j,'vo1');
          const isFullyDone=j.stage==='job_complete';
  ...
  // Claim Batches section — show all batches generated by Finance
  const batchEl=document.getElementById('m-claim-batches');
  if(batchEl){
    const batches=Object.entries(DB.batchDocs||{});
    if(!batches.length){
      batchEl.innerHTML='<div style="padding:1.5rem;text-align:center;color:var(--tx3);font-size:.8rem">No claim batches yet — Finance generates these after GIS documents are uploaded</div>';
    } else {
      batchEl.innerHTML=batches.map(([certNo,b])=>{
        const bWOs=b.wos||[];
        ...
        const scans=b.scans||{};
        // helper: get doc from batch — keys may be camelCase or snake_case
        ...
        const docDefs=[
          {dt:'annexure',lbl:'Annexure'},
          {dt:'payment_cert',lbl:'Payment Cert'},
          {dt:'invoice',lbl:'Invoice'},
          {dt:'list_of_jobs',lbl:'List of Jobs'},
          {dt:'bpc_spreadsheet',lbl:'BPC Sheet'},
        ];
        const docBtns=docDefs.map(d=>{
          if(scans[d.dt]) return`<button class="btn btn-gn btn-sm" style="font-size:.72rem" onclick="downloadScan(null,'${d.dt}','${certNo}')">&#11015;${d.lbl}&#10003;</button>`;
          if(bget(d.dt)) return`<button class="btn btn-bl btn-sm" style="font-size:.72rem" onclick="viewBatchDoc('${certNo}','${d.dt}')">&#11015; ${d.lbl}</button>`;
          return`<span style="font-size:.7rem;color:var(--tx3);padding:2px 6px;border:1px solid var(--bd);border-radius:3px;opacity:.4">${d.lbl}</span>`;
        }).join(' ');
        return`<div style="padding:.8rem 1rem;border-bottom:1px solid var(--bd)">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
            <span style="font-size:.82rem;font-weight:700;color:var(--tx)">Cert ${certNo}</span>
            <span style="font-size:.75rem;color:var(--tx2)">${bWOs.length} job${bWOs.length>1?'s':''}</span>
            <span class="mono" style="font-size:.75rem;color:var(--am);margin-left:auto">${P(bTotal)}</span>
          </div>
          <div style="font-size:.72rem;color:var(--tx3);margin-bottom:7px">WOs: ${bWOs.join(', ')}</div>
          <div style="display:flex;flex-wrap:wrap;gap:5px">${docBtns}</div>
        </div>`;
      }).join('');
    }
  }
    ...
    return`<div style="display:grid;grid-template-columns:80px 1fr 80px 100px 1fr 80px;gap:10px;padding:.6rem 1rem;border-bottom:1px solid var(--bd);font-size:.8rem;align-items:center;cursor:pointer;transition:.1s" onclick="openJobDetail('${j.wo}')" onmouseover="this.style.background='var(--sf2)'" onmouseout="this.style.background=''">
      ...
      <div>
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-top:3px">
          <div class="prog-w" style="width:100px"><div class="prog-b" style="width:${pct}%;background:${pct===100?'var(--gn)':'var(--am)'}"></div></div>
          <span style="font-size:.62rem;color:var(--tx3)">${pct}%</span>
        </div>
      </div>
      <span class="tag">${j.loc.split(',')[0]}</span>
      <span class="mono">${P(t.total)}</span>
