...
const STAGES = [
  ...
  {id:'gis_notified_early',       lbl:'GIS Consultant Notified (After Linesman)',   role:'admin'},
  {id:'gis_complete_early',       lbl:'GIS Map Received (After Linesman)',        role:'admin'},
  ...
  {id:'gis_notified',             lbl:'GIS Consultant Notified (Final)',           role:'admin'},
  {id:'gis_complete',             lbl:'GIS Map Received & Uploaded (Final)',     role:'admin'},
  ...
  {id:'work_order_deleted',       lbl:'Work Order Deleted from System',            role:'admin'},
];
...
const STAGE_DOCS = {
  ...
  gis_notified_early:'gis_report', gis_complete_early:'gis_report',
...
function renderMDDash(){
  ...
  const ST_LBL={wo_received:'Awaiting VO1',vo1_created:'Linesman Notification',linesman_notified:'Linesman Survey',field_received:'Awaiting Early GIS',gis_notified_early:'Early GIS Notification',gis_complete_early:'Early GIS Complete',vo2_created:'Creating VO2',works_valuation_created:'Works Instruction',work_complete:'GIS Notification',gis_notified:'GIS On Site',gis_complete:'Awaiting Claim Docs',claim_docs_ready:'Pending Completion',job_complete:'Complete',work_order_deleted:'Deleted'};
  ...
  wwEl.innerHTML=`
    ...
    <div id="md-panel-batches" class="cd-panel" style="display:none">
      <div class="cd-panel-hd"><span>Batch Documents Generated by Finance <span class="badge b-gy">${(() => {
        const financeDocTypes = ['annexure', 'payment_cert', 'invoice', 'list_of_jobs', 'bpc_spreadsheet'];
        const allFinanceDocs = Object.values(DB.jobs).filter(j => j.stage === 'gis_complete' || j.stage === 'claim_docs_ready')
    ...
    if(!isMD){
      if(canEdit){
        ...
        if(st.id==='gis_notified') {
          ...
          actBtns+=`<button class="btn btn-am" onclick="openExternalUpload('${wo}','gis_report')">📎 Upload GIS Maps (${hasGISFiles?'1 file':''})</button>`;
      ...
      // Add delete button for all roles on all stages
      actBtns+=`<button class="btn btn-rd btn-sm" onclick="confirmDeleteWorkOrder('${wo}')" style="margin-left:.5rem;background:#dc3545">🗑 Delete WO</button>`;
    }
...
function docAnnexure(job, batchJobs){
  ...
  const inWrap=(val)=>`<div contenteditable="true" style="width:100%;min-height:1.2em;white-space:normal;word-break:break-word;overflow-wrap:anywhere;font-family:Arial,sans-serif;font-size:8.5pt;color:#000;padding:0 2px;outline:none">${(val||'').replace(/</g,'&lt;')}</div>`;
  ...
  <table style="width:100%;min-width:1500px;border-collapse:collapse;font-size:9pt;margin-top:6px;table-layout:auto">
    ...
    <thead>
      <tr style="background:#d9d9d9">
        ...
        <th style="border:1px solid #999;padding:2px 3px;text-align:center;overflow:visible;white-space:normal">% COMPLETE</th>
        ...
        <th style="border:1px solid #999;padding:2px 3px;overflow:visible;white-space:normal">PLANNED START DATE</th>
        <th style="border:1px solid #999;padding:2px 3px;overflow:visible;white-space:normal">ACTUAL COMPLETION DATE</th>
...
const docLabels={
  ...
  works_instruction:'Works Instruction',gis_report:'GIS Map',gis_cert:'GIS Certificate',
