...
/* ═══════════════════════════════════════════════════
   PIPELINE — visual journey cards (replaces old dots)
   ═══════════════════════════════════════════════════ */

/* Legacy dot classes kept for non-pipeline lists (tasks, etc.) */
...
@keyframes pulse{0%,100%{box-shadow:0 0 0 0 var(--am-bg)}50%{box-shadow:0 0 0 6px transparent}}
...
/* ── NEW: pipeline journey cards ── */
#jdPipeline{
  display:flex;gap:0;overflow-x:auto;
  padding:1.5rem 1rem 1.2rem!important;
  scrollbar-width:thin;scrollbar-color:var(--bd2) transparent;
  /* subtle gradient fade at edges to hint scrollability */
  -webkit-mask-image:linear-gradient(90deg,transparent 0,#000 2.5%,#000 97.5%,transparent 100%);
  mask-image:linear-gradient(90deg,transparent 0,#000 2.5%,#000 97.5%,transparent 100%);
}
#jdPipeline::-webkit-scrollbar{height:4px}
#jdPipeline::-webkit-scrollbar-track{background:transparent}
#jdPipeline::-webkit-scrollbar-thumb{background:var(--bd2);border-radius:2px}

/* Each stage = a card */
#jdPipeline .pipe-step{
  flex-direction:column;align-items:center;text-align:center;
  min-width:120px;max-width:148px;flex:1 0 120px;
  padding:0;margin-bottom:0;position:relative;background:none;
}
#jdPipeline .pipe-step:hover{background:none}

/* connector track between cards */
#jdPipeline .pipe-step:not(:last-child)::after{
  content:'';position:absolute;top:22px;
  left:calc(50% + 22px);right:calc(-50% + 22px);
  height:2px;background:var(--bd2);z-index:0;
  transition:background .35s;
}
#jdPipeline .pipe-step:has(.pipe-dot.done):not(:last-child)::after{background:var(--gn)}

/* The circle icon at top of each card */
#jdPipeline .pipe-dot{
  width:44px;height:44px;border-radius:50%;
  border:2px solid var(--bd2);
  display:flex;align-items:center;justify-content:center;
  flex-shrink:0;font-size:1.1rem;margin-top:0;z-index:1;
  background:var(--sf2);position:relative;
  transition:transform .2s,box-shadow .2s,border-color .2s,background .2s;
  cursor:default;
}
#jdPipeline .pipe-dot.done{
  background:var(--gn-bg);border-color:var(--gn);color:var(--gn);font-size:.9rem;
}
#jdPipeline .pipe-dot.active{
  background:var(--am-bg);border-color:var(--am);color:var(--am);font-size:.9rem;
  animation:cardPulse 2.2s ease-in-out infinite;
}
#jdPipeline .pipe-dot.pending{
  background:var(--sf);border-color:var(--bd2);color:var(--tx3);font-size:.9rem;
}
@keyframes cardPulse{
  0%,100%{box-shadow:0 0 0 0 rgba(240,165,0,0)}
  50%{box-shadow:0 0 0 7px rgba(240,165,0,.12)}
}

/* Clickable card dot for steps with actions */
#jdPipeline .pipe-dot[data-has-action="true"]{cursor:pointer}
#jdPipeline .pipe-dot[data-has-action="true"]:hover{
  transform:scale(1.12);
  box-shadow:0 4px 18px rgba(240,165,0,.28);
  border-color:var(--am);
}
#jdPipeline .pipe-dot.done[data-has-action="true"]:hover{
  box-shadow:0 4px 14px rgba(29,184,122,.25);
  border-color:var(--gn);
}

/* text below each circle */
#jdPipeline .pipe-info{
  flex:none;width:100%;margin-top:8px;padding:0 4px;
}
#jdPipeline .pipe-lbl{
  font-size:.67rem;line-height:1.3;max-width:130px;margin:0 auto;
  font-weight:500;
}
#jdPipeline .pipe-lbl.active{font-weight:700;color:var(--am)}
#jdPipeline .pipe-lbl.done{color:var(--tx2)}
#jdPipeline .pipe-lbl.pending{color:var(--tx3)}
#jdPipeline .pipe-date{font-size:.58rem;margin-top:3px;color:var(--gn);font-weight:500}

/* "tap to act" nudge badge under active step */
#jdPipeline .pipe-nudge{
  display:inline-block;margin-top:5px;
  font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;
  color:var(--am);background:var(--am-bg);border:1px solid var(--am-b);
  border-radius:20px;padding:.15rem .55rem;
  animation:nudgeFade 1.8s ease-in-out infinite;
}
@keyframes nudgeFade{0%,100%{opacity:.6}50%{opacity:1}}
...
.pipe-popup-overlay.open{display:flex;animation:popFadeIn .22s ease}
...
.pipe-popup{
  ...
  animation:popSlideUp .22s cubic-bezier(.34,1.56,.64,1);
  position:relative;
}
@keyframes popSlideUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}
...
.pipe-popup-icon.done{background:var(--gn-bg);border:1px solid var(--gn-b)}
.pipe-popup-icon.active{background:var(--am-bg);border:1px solid var(--am-b)}
.pipe-popup-icon.pending{background:var(--sf2);border:1px solid var(--bd)}

.pipe-popup-title{
  font-family:var(--font-display);font-weight:700;font-size:1rem;
  color:var(--tx);line-height:1.2;margin-bottom:3px;
}
.pipe-popup-subtitle{font-size:.72rem;color:var(--tx3);line-height:1.4}
.pipe-popup-status{
  display:inline-flex;align-items:center;gap:5px;
  margin-top:6px;font-size:.65rem;font-weight:700;text-transform:uppercase;
  letter-spacing:.09em;padding:.2rem .6rem;border-radius:20px;
}
.pipe-popup-status.done{background:var(--gn-bg);border:1px solid var(--gn-b);color:var(--gn)}
.pipe-popup-status.active{background:var(--am-bg);border:1px solid var(--am-b);color:var(--am)}
.pipe-popup-status.pending{background:var(--sf2);border:1px solid var(--bd);color:var(--tx3)}

.pipe-popup-body{padding:1.2rem 1.5rem}
.pipe-popup-body .pipe-actions{flex-direction:column;gap:8px}
.pipe-popup-body .pipe-actions .btn{width:100%;justify-content:center;padding:.65rem 1rem;font-size:.82rem}
.pipe-popup-body .pipe-actions > div{width:100%;box-sizing:border-box}

.pipe-popup-close{
  position:absolute;top:1rem;right:1rem;
  background:var(--sf2);border:1px solid var(--bd);border-radius:50%;
  width:28px;height:28px;display:flex;align-items:center;justify-content:center;
  cursor:pointer;color:var(--tx2);font-size:.9rem;
  transition:background .15s,color .15s,border-color .15s;
}
.pipe-popup-close:hover{background:var(--rd-bg);border-color:var(--rd-b);color:var(--rd)}

.pipe-popup-info-row{
  display:flex;align-items:center;gap:8px;padding:.55rem 0;
  border-bottom:1px solid var(--bd);font-size:.78rem;
}
.pipe-popup-info-row:last-child{border-bottom:none}
.pipe-popup-info-row .lbl{color:var(--tx3);min-width:100px;flex-shrink:0}
.pipe-popup-info-row .val{color:var(--tx);font-weight:500}

/* Progress trail at top of popup */
.pipe-popup-progress{
  height:3px;border-radius:3px 3px 0 0;
  background:linear-gradient(90deg,var(--gn),var(--am));
  transition:width .4s;
}
...
@media(max-width:820px){
  ...
  #jdPipeline .pipe-step{
    ...
    min-width:0;max-width:none;flex:none;gap:14px;padding:.5rem 0;
  }
  #jdPipeline .pipe-step:not(:last-child)::after{
    top:44px;bottom:-4px;left:21px;right:auto;
    width:2px;height:auto;
  }
  #jdPipeline .pipe-dot{width:44px;height:44px;flex-shrink:0}
  #jdPipeline .pipe-info{margin-top:4px;width:auto;flex:1}
  #jdPipeline .pipe-lbl{max-width:none;margin:0;font-size:.82rem}
  #jdPipeline .pipe-nudge{margin-top:4px}
}
...
.doc-card:hover{border-color:var(--bd2);box-shadow:0 2px 12px rgba(0,0,0,.2)}
.doc-card-top{display:flex;align-items:center;gap:10px;margin-bottom:8px}
.doc-card-icon{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:.85rem}
...
/* ============================================================
   WORKFLOW PIPELINE overrides removed — replaced by new
   visual card pipeline above (search: NEW: pipeline journey)
   ============================================================ */
...
/* ============================================================
   BUSINESS UI POLISH — buttons, task items, card headers
   ============================================================ */

/* Larger, friendlier primary action buttons */
.btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;
  border:none;border-radius:var(--rs);cursor:pointer;font-family:var(--font-body);
  font-weight:600;transition:filter .15s,transform .1s,box-shadow .15s;letter-spacing:.01em}
.btn:active{transform:translateY(1px)}
.btn-am{background:var(--am);color:#0a0f1c;padding:.6rem 1.1rem;font-size:.82rem}
.btn-am:hover{filter:brightness(1.08);box-shadow:0 6px 18px rgba(240,165,0,.28)}
.btn-gn{background:var(--gn);color:#fff;padding:.6rem 1.1rem;font-size:.82rem}
.btn-gn:hover{filter:brightness(1.08);box-shadow:0 6px 18px rgba(29,184,122,.28)}
.btn-bl{background:var(--bl-bg);border:1px solid var(--bl-b);color:var(--bl);padding:.5rem .95rem;font-size:.79rem}
.btn-bl:hover{background:var(--bl);color:#fff}
.btn-gy{background:var(--sf2);border:1px solid var(--bd2);color:var(--tx2);padding:.5rem .95rem;font-size:.79rem}
.btn-gy:hover{border-color:var(--tx2);color:var(--tx)}
.btn-rd{background:var(--rd-bg);border:1px solid var(--rd-b);color:var(--rd);padding:.5rem .95rem;font-size:.79rem}
.btn-rd:hover{background:var(--rd);color:#fff}
.btn-sm{padding:.38rem .75rem!important;font-size:.74rem!important}

/* Task + waiting items on dashboard — more card-like */
.task-item{
...
.task-item:hover{
  border-color:var(--am-b);
  box-shadow:0 3px 14px rgba(240,165,0,.12);
  transform:translateX(3px);
}

/* Card (crd) refinements */
.crd{background:var(--sf);border:1px solid var(--bd);border-radius:var(--r);overflow:hidden}
.crd-hd{display:flex;align-items:center;gap:8px;padding:.8rem 1.1rem;
  border-bottom:1px solid var(--bd);font-size:.78rem;font-weight:600;
  background:linear-gradient(90deg,var(--sf2),var(--sf))}
.crd-hd-l{display:flex;align-items:center;gap:7px;flex:1}

/* Stats row */
.srow{display:grid;grid-template-columns:repeat(4,1fr);gap:.75rem;margin-bottom:1.25rem}
@media(max-width:760px){.srow{grid-template-columns:repeat(2,1fr)}}
@media(max-width:480px){.srow{grid-template-columns:1fr}}
...
.sc::after{
  ...
  background:linear-gradient(90deg,var(--am),var(--gn));
  opacity:0;transition:opacity .2s;
}
.sc:hover{border-color:var(--bd2);box-shadow:0 4px 20px rgba(0,0,0,.3)}
.sc:hover::after{opacity:1}
/* remove old ::before accent line if still active */
.sc::before{display:none}

/* Back button in job detail */
#sc-jobdetail > button{
  background:var(--sf2);border:1px solid var(--bd);border-radius:20px;
  padding:.38rem 1rem;color:var(--tx2);font-family:var(--font-body);font-size:.75rem;
  cursor:pointer;display:inline-flex;align-items:center;gap:6px;
  margin-bottom:1rem;transition:.12s;
}
#sc-jobdetail > button:hover{border-color:var(--am);color:var(--am);background:var(--am-bg)}

/* Badge polish */
.badge{display:inline-flex;align-items:center;padding:.2rem .6rem;
  border-radius:20px;font-size:.65rem;font-weight:700;letter-spacing:.04em;white-space:nowrap}
.b-am{background:var(--am-bg);border:1px solid var(--am-b);color:var(--am)}
.b-gn{background:var(--gn-bg);border:1px solid var(--gn-b);color:var(--gn)}
.b-bl{background:var(--bl-bg);border:1px solid var(--bl-b);color:var(--bl)}
.b-rd{background:var(--rd-bg);border:1px solid var(--rd-b);color:var(--rd)}
.b-gy{background:var(--sf2);border:1px solid var(--bd2);color:var(--tx3)}

/* Light mode popup tweak */
[data-theme="light"] .pipe-popup{
  background:#fff;border-color:#c4ccdc;
  box-shadow:0 24px 60px rgba(0,0,0,.18);
}
[data-theme="light"] .pipe-popup-head{border-bottom-color:#c4ccdc}
[data-theme="light"] .pipe-popup-info-row{border-bottom-color:#c4ccdc}
